...
This is the GitHub organization profile repository for MailZeet, a transactional email service that is an entity of Axa Zara. It contains the public-facing organization profile page (`profile/README.md`) displayed on the MailZeet GitHub organization page, plus a root-level repository description. There are no application source files, build scripts, or dependencies — this repo is purely a GitHub org-profile repo.

## Tech stack

- Markdown (GitHub Flavored Markdown for the profile page)

## Architecture

The repository has a minimal two-file structure:

- `README.md` — one-line repository title (`# .github`)
- `profile/README.md` — the organization profile page rendered by GitHub at the top of the MailZeet organization; describes MailZeet's features (Drag and Drop editor, personalization, tracking, collaboration, API), lists SDK integrations (PHP, Laravel), and links to documentation at `docs.mailzeet.com`

There are no workflows, CI pipelines, issue templates, or community health files present.

## Conventions

- All content is plain Markdown; no tooling or linting is configured
- The `profile/README.md` file is the only publicly visible artifact — edits here directly update the GitHub organization landing page
- Badge images use `shields.io` with `for-the-badge` style
- External links reference `mailzeet.com`, `docs.mailzeet.com`, `axazara.com`, and `careers.axazara.com`
